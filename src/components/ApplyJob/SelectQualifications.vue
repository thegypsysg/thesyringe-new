            ...
            >
              <v-row>
                <v-col cols="12">
                  ...
                  <h4 class="mb-8">Please select any one of the highest qualifications you hold</h4>

                  <!-- <v-form  fast-fail @submit.prevent="login"> -->
                    <div class="position-relative">
                    <v-radio-group class="w-100" v-model="qualification" inline>
                      <v-radio
                        v-for="option in resource.qualifications"
                        :key="option.value"
                        :label="option.label"
                        :value="option.value"
                        class="w-100 py-2"
                      >
                        <template #label>
                          <p class="font-weight-bold text-red-darken-4" style="line-height: 18px;">{{ option.label }}</p>
...
export default {
  ...
  mounted() {
    ...
    // this.qualification = qtId ? qtId : applicantData.sgm_id;
    this.qualification = qtId;
  ...
  methods: {
    saveData() {
      const payload = {
        qt_id: this.qualification,
      };
      //console.log(payload);
      const token = localStorage.getItem("token");
      if(this.qualification) {
      axios
        .post(`/gypsy-applicant/save-qualification-type`, payload, {
          headers: {
        ...
        .then((response) => {
          ...
          localStorage.setItem("qt_id", data.data.qt_id);
          const qualificationType = this.resource.qualifications.filter(sk => sk.value == data.data.qt_id)[0].label
          console.log(qualificationType)
          localStorage.setItem("qualification_type", qualificationType);
          this.nextStep()
        })
        .catch((error) => {
